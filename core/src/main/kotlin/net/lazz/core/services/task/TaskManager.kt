package net.lazz.core.services.task
...
import net.lazz.core.services.task.enum.TaskState
import net.lazz.core.services.task.manager.AsyncTaskManager
import net.lazz.core.services.task.manager.SyncTaskManager
import net.lazz.core.services.task.monitor.TaskProfiler
import org.bukkit.Bukkit
import kotlin.time.Duration

@Service
class TaskManager(private val plugin: Core) {
    ...
    private lateinit var profiler: TaskProfiler
    private lateinit var sync: SyncTaskManager
    private lateinit var async: AsyncTaskManager
    ...
    fun onEnable() {
        ...
        sync = SyncTaskManager(plugin, profiler) { logs }
        async = AsyncTaskManager(profiler, { logs })
        plugin.logger.info("TaskManager iniciado")
    }

    @OnDisable
    fun onDisable() {
        shutdown()
        plugin.logger.info("TaskManager desligado")
    }
